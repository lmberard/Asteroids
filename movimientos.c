...
void verificar_limites_pantalla(float *posicion_x, float *posicion_y){
	...
	if(*posicion_y >= VENTANA_ALTO)
		*posicion_y = 10;

	if(*posicion_y <=0)
		*posicion_y = VENTANA_ALTO -10;
}
...
void modificar_parametros_nave(float *x_anterior, float *posicion_x, float *posicion_y, float *velocidad_y, float *velocidad_x, float *angulo, float *dt, float *potencia){
			...
			*velocidad_y = computar_velocidad(*velocidad_y, *potencia*cos(*angulo), *dt);
